...
func call(fn reflect.Value, args ...reflect.Value) (reflect.Value, error) {
	...
	//fmt.Println("call2:", argv)
	//fmt.Println("call2:", fn.Kind(), fn.Type(), v.Kind(), v.Type())
	result := v.Call(argv)
	//fmt.Println("call3:", result)
	//fmt.Println("call3:", result[0].Kind(), result[0].Type())
	rlen	:= len(result)
	if rlen == 0 {
	...
	case 2:
        if result[1].Kind() == reflect.Interface {
            if !result[1].IsNil() {
            	//a, err := 1, error
            	//a, ok := 1, bool
                if e, ok := result[1].Interface().(error); ok {
            	    return val, e
                }
            }else{
            	//a, err := 1, nil
        ...
        //有两个参数，最后一个参数是非error类型，跳到下面。
        fallthrough
	default:
        var cv []interface{}
       	var results = reflect.ValueOf(&cv).Elem()
        return reflect.Append(results, result...), nil
    }
/*

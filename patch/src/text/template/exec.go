...
func (s *state) evalCall(dot, fun reflect.Value, node parse.Node, name string, args []parse.Node, final reflect.Value) reflect.Value {
	//fmt.Println("evalCall", dot, fun, node, name, args, final)
	if args != nil {
		args = args[1:] // Zeroth arg是函数名/节点;没有传递给功能。
	}
	typ := fun.Type()
	numIn := len(args)
	if final != missingVal {
	...
	numFixed := len(args)
	if typ.IsVariadic() {
		numFixed = typ.NumIn() - 1 // 最后一个arg是可变参数。
		if numIn < numFixed {
	...
	//fmt.Println("evalCall2-1:", node, name, args, argv)
	//fmt.Println("evalCall2-2:", fun, fun.IsValid(), fun.Kind(), fun.Type(), fun.Type().Name(), fun.Type().PkgPath())
	result 	:= fun.Call(argv)
	rlen	:= len(result)
	//fmt.Println("evalCall2-3:", result, rlen)
	if rlen == 0 {
		//空字符
    	return reflect.ValueOf("")
	}
	val := result[0]
	//fmt.Println("evalCall4:", val, val.IsValid(), val.Kind(), val.Type(), val.Type().Name(), val.Type().PkgPath())
	_,isBuiltin := builtins[name]
	if isBuiltin && val.Type() == reflectValueType{
		val = val.Interface().(reflect.Value)
		//fmt.Println("evalCall5:", val, val.IsValid(), val.Kind(), val.Type(), val.Type().Name(), val.Type().PkgPath())
	}
    switch rlen{
	case 1:
		//一个返回值
	...
	case 2:
        ...
        if result[1].Kind() == reflect.Interface {
            if result[1].IsNil() {
            	//空表示没有错误
                return val
            }else if _, ok := result[1].Interface().(error); ok {
            	//返回错误
        	    return result[1]
            }
        }
	...
	default:
		//两个或更多返回值
        var cv []interface{}
       	var results = reflect.ValueOf(&cv).Elem()
       	//if isBuiltin {
	    //	for i, v := range result {
		//		if v.Type() == reflectValueType {
		//			fi := v.Interface().(reflect.Value)
		//			if fi.Type() == reflectValueType && result[i].CanInterface() {
		//				results = reflect.Append(results, result[i].Interface())
		//				continue
		//			}
		//		}
	    ...
	    //	return results
       	//}
        return reflect.Append(results, result...)
    }
/*
	...
	v := result[0]
	fmt.Println("evalCall4:", v, v.IsValid(), v.Kind(), v.Type(), v.Type().Name(), v.Type().PkgPath())
	if v.Type() == reflectValueType {
		v = v.Interface().(reflect.Value)
	}
	fmt.Println("evalCall5:", v, v.IsValid(), v.Kind(), v.Type(), v.Type().Name(), v.Type().PkgPath())
	return v
*/
}
